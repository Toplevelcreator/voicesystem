...
const GITHUB_REPO = 'jamiepine/voicetoto';
...
export async function getLatestRelease(): Promise<ReleaseInfo> {
  ...
  try {
    ...
    const releaseInfo: ReleaseInfo = {
      version,
      downloadLinks: {
        macArm: downloadLinks.macArm || `${baseUrl}/voicetoto_aarch64.app.tar.gz`,
        macIntel: downloadLinks.macIntel || `${baseUrl}/voicetoto_x64.app.tar.gz`,
        windows:
          downloadLinks.windows || `${baseUrl}/voicetoto_${version.replace('v', '')}_x64_en-US.msi`,
        linux: downloadLinks.linux || `${baseUrl}/voicetoto_x86_64-unknown-linux-gnu.AppImage`,
      },
    };
