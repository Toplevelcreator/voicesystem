...
**Issue:** [#10 OpenAI API compatibility](https://github.com/jamiepine/voicetoto/issues/10)

## Overview

This feature exposes OpenAI-compatible endpoints from Voicetoto, allowing any tool, library, or application that speaks the OpenAI Audio API to use Voicetoto as a drop-in local replacement.
...
flowchart LR
    ...
    subgraph voicetoto [Voicetoto Server]
...
- **OpenAI SDK users**: `openai.audio.speech.create()` works with Voicetoto
- **LLM frameworks**: LangChain, AutoGen, etc. can use Voicetoto for TTS
...
- `voice` parameter maps to Voicetoto profile names (case-insensitive)
...
async def resolve_voice_for_openai(voice: str, db: Session) -> Optional[VoiceProfile]:
    """
    Resolve OpenAI voice parameter to a Voicetoto profile.
