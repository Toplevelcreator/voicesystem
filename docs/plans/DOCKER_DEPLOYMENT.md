...
Docker support makes Voicetoto easier to deploy, especially for:
...
```bash
# CPU-only version
docker run -p 8000:8000 -v voicetoto-data:/app/data \
  ghcr.io/jamiepine/voicetoto:latest

# NVIDIA GPU version
docker run --gpus all -p 8000:8000 -v voicetoto-data:/app/data \
  ghcr.io/jamiepine/voicetoto:latest-cuda

# AMD GPU version (experimental)
docker run --device=/dev/kfd --device=/dev/dri -p 8000:8000 \
  -v voicetoto-data:/app/data \
  ghcr.io/jamiepine/voicetoto:latest-rocm
```
...
services:
  voicetoto:
    image: ghcr.io/jamiepine/voicetoto:latest-cuda
    ports:
      - "8000:8000"
    volumes:
      - voicetoto-data:/app/data
...
Build and run:
```bash
docker build -t voicetoto .
docker run -p 8000:8000 -v $(pwd)/data:/app/data voicetoto
```
...
Build:
```bash
docker build -f Dockerfile.optimized -t voicetoto:slim .
```
...
docker run --device=/dev/kfd --device=/dev/dri \
  ...
  -p 8000:8000 -v voicetoto-data:/app/data \
  voicetoto:rocm
```
...
```bash
docker run -v voicetoto-data:/app/data \           # Profiles, generations, history
           -v huggingface-cache:/root/.cache/huggingface \  # Downloaded models
           -p 8000:8000 voicetoto
```
...
Configure Voicetoto via environment variables:
...
doctl compute droplet create voicetoto \
...
sh get-docker.sh
docker run --gpus all -d -p 80:8000 voicetoto:cuda
```
...
```bash
# Build and push
docker build -t gcr.io/your-project/voicetoto .
docker push gcr.io/your-project/voicetoto

# Deploy to Cloud Run
gcloud run deploy voicetoto \
  --image gcr.io/your-project/voicetoto \
  --platform managed \
  --region us-central1 \
  --memory 4Gi \
  --cpu 2 \
...
[build]
  image = "ghcr.io/jamiepine/voicetoto:latest"
...
```bash
# Fix permissions
docker run --user $(id -u):$(id -g) -v $(pwd)/data:/app/data voicetoto
```
...
```bash
docker run -e GPU_MEMORY_FRACTION=0.5 voicetoto
```

Or use CPU-only:
```bash
docker run -e DEVICE=cpu voicetoto
```
...
```bash
docker run -v huggingface-cache:/root/.cache/huggingface voicetoto
```

Or use host cache:
```bash
docker run -v ~/.cache/huggingface:/root/.cache/huggingface voicetoto
```
...
Change host port:
```bash
docker run -p 8080:8000 voicetoto  # Use port 8080 instead
```
...
```dockerfile
RUN useradd -m -u 1000 voicetoto
USER voicetoto
```
...
docker run -e GPU_MEMORY_FRACTION=0.8 voicetoto

# Allow GPU memory growth (prevents OOM)
docker run -e TF_FORCE_GPU_ALLOW_GROWTH=true voicetoto
```
...
View container logs:
```bash
docker logs -f voicetoto

# Or with compose
docker compose logs -f voicetoto
```
...
4. Report issues: [GitHub Issues](https://github.com/jamiepine/voicetoto/issues)
