...
This creates:
- Private key: `~/.tauri/voicetoto.key` (keep secret!)
- Public key: `~/.tauri/voicetoto.key.pub` (safe to share)

### 2. Update Tauri Config

Open `tauri/src-tauri/tauri.conf.json` and:

1. Replace `"REPLACE_WITH_YOUR_PUBLIC_KEY"` with the content from `~/.tauri/voicetoto.key.pub`
...
- Make sure you copied the entire content from `voicetoto.key.pub`
...
- Verify private key file exists at `~/.tauri/voicetoto.key`
